...
module.exports = async (req, res) => {
    ...
    if (!id) {
        // Si no viene el ID, devolver un error claro
        console.error("Solicitud a factura-ticket sin parámetro 'id'");
    ...
    console.log(`[Factura Ticket] Iniciando generación para Venta ID: ${saleId}`);
    ...
    try {
        ...
        // 2. Datos de la Venta y Cliente (JOIN, incluyendo DESCUENTO global de VENTA)
        console.log(`[Factura Ticket ${saleId}] Obteniendo datos de venta y cliente...`);
        const saleSql = `
            SELECT
                v.*,
                v.DESCUENTO as DESCUENTO_GLOBAL, {/* Alias para el descuento de la tabla VENTA */}
                c.CLIENTE,
        ...
        detailsData.forEach((item, index) => {
            ...
            const subtotalItemNeto = (cantidad * precioUnitario) - descuentoItem; // Total de línea neto
        ...
        // Calcular el Total Venta final (como en AppSheet: Subtotal Neto - Descuento Global)
        const totalVentaFinal = subTotalNetoCalculado - descuentoGlobalVenta;
        console.log(`[Factura Ticket ${saleId}] Cálculos: SubTotalNeto=${subTotalNetoCalculado}, DescItems=${descuentoItemsCalculado}, DescGlobal=${descuentoGlobalVenta}, TotalFinal=${totalVentaFinal}`);

        // Formatear Fecha y Hora
        const fechaVenta = saleData['FECHA DE VENTA'] ? new Date(saleData['FECHA DE VENTA']).toLocaleDateString('es-ES') : 'N/A';
        const horaVenta = saleData['HORA VENTA'] || ''; // Asume que viene formateada de la BD
...
<body>
  <div class="invoice-box">
    ...
    {/* La tabla empieza aquí, sin comentarios stray */}
    <table>
      <thead>
        <tr class="heading">
          <td>Descrip</td>
      ...
      <tfoot>
        {/* Pie de página alineado con lógica AppSheet */}
        <tr class="desc">
  ...
  <script>
    ...
    window.onload = function () {
      try {
          ...
          // Añadir mensaje y botón de impresión manual si falla la automática
